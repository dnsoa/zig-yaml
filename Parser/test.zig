...
fn parseError2(source: []const u8, comptime format: []const u8, args: anytype) !void {
    ...
    defer given.deinit();
    try bundle.renderToWriter(.{ .include_reference_trace = false }, &given.writer);
...
test "flow mapping parses" {
    ...
    try parseSuccess("{}");
    try parseSuccess("{ }");
    try parseSuccess("{ a: 1, b: 2, }");
}

test "unterminated quoted strings are errors" {
    try parseError("\"unterminated", error.UnterminatedString);
    try parseError("'unterminated", error.UnterminatedString);
    try parseError("key: \"unterminated", error.UnterminatedString);
    try parseError("key: 'unterminated", error.UnterminatedString);
}

test "deeply nested map does not overrun the token stream" {
    // Regression: value() used to leave the iterator advanced past the
    // terminal eof when a key had no value, crashing on deep nesting.
    try parseSuccess(
        \\a:
        \\ b:
        \\  c:
...
test "flow mapping with non-literal key is an error" {
    try parseError("{ [a]: 1 }", error.ParseFailure);
}
