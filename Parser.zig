...
fn value(self: *Parser, gpa: Allocator) ParseError!Node.OptionalIndex {
    ...
    switch (tok.id) {
        ...
        // No value here: restore the iterator to where we started so callers
        // observe a clean "no value consumed" position. Without this the next
        // token (often the terminal eof) is silently skipped, which can push a
        // caller's read position past the end of the token stream.
        else => {
            self.token_it.seekTo(pos);
            return .none;
        },
...
fn doc(self: *Parser, gpa: Allocator) ParseError!Node.Index {
    ...
    // Parse value. `value` already restores the iterator position when it
    // finds no value, so no manual compensation is needed here.
...
fn map(self: *Parser, gpa: Allocator) ParseError!Node.OptionalIndex {
    ...
    try self.encodeMap(gpa, @enumFromInt(node_index), entries.items, .{
        .start = node_start,
        .end = node_end,
    });
...
fn encodeMap(
    ...
    if (entries.len == 1) {
        ...
        try self.extra.ensureUnusedCapacity(gpa, entries.len * 2 + 1);
        const extra_index: u32 = @intCast(self.extra.items.len);

        _ = self.addExtraAssumeCapacity(Map{ .map_len = @intCast(entries.len) });
...
fn mapBracketed(self: *Parser, gpa: Allocator) ParseError!Node.OptionalIndex {
    ...
    log.debug("(flow map) begin {s}@{d}", .{ @tagName(self.token(node_start).id), node_start });
    ...
    const node_end: Token.Index = while (true) {
        ...
        const key = self.token_it.next() orelse return error.UnexpectedEof;
        if (key.id != .literal) {
            return self.fail(gpa, key_pos, "expected key in flow mapping", .{});
        }
        ...
        // Parse value (may be empty, e.g. `{ a: , b: 1 }`)
        const value_index = try self.value(gpa);
    ...
    log.debug("(flow map) end {s}@{d}", .{ @tagName(self.token(node_end).id), node_end });

    try self.encodeMap(gpa, node_index, entries.items, .{
        .start = node_start,
        .end = node_end,
    });
...
fn eatCommentsAndSpace(self: *Parser, comptime exclusions: []const Token.Id) void {
    ...
    outer: while (self.token_it.next()) |tok| {
        ...
        switch (tok.id) {
            .comment, .space, .tab, .new_line => |space| {
...
fn parseSingleQuoted(self: *Parser, gpa: Allocator, raw: []const u8) ParseError!String {
    // `raw.len < 2` or a missing closing quote means the scalar was never
    // terminated (e.g. the buffer ended mid-string).
    ...
    while (index < raw_no_quotes.len) : (index += 1) {
        ...
        switch (state) {
            ...
            .escape => {
                ...
                state = .start;
                self.string_bytes.appendAssumeCapacity(unescaped);
                string.len += 1;
            },
        }
    }
