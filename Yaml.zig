...
pub fn parse(self: Yaml, arena: Allocator, comptime T: type) Error!T {
    ...
    if (self.docs.items.len == 1) {
        if (self.parseValue(arena, T, self.docs.items[0])) |parsed| {
            return parsed;
        } else |err| switch (err) {
            // A single document can still be requested as a list of documents
            // (`[]T` / `[1]T`); fall through to the multi-doc handling below
            // rather than failing outright.
            error.TypeMismatch => switch (@typeInfo(T)) {
                .array, .pointer => {},
                else => return err,
            },
            else => return err,
    ...
    switch (@typeInfo(T)) {
        .array => |info| {
            if (info.len != self.docs.items.len) return error.ArraySizeMismatch;
...
fn parseValue(self: Yaml, arena: Allocator, comptime T: type, value: Value) Error!T {
    return switch (@typeInfo(T)) {
        ...
        // An empty node maps to `null`; otherwise parse the wrapped type.
        .optional => |info| if (value == .empty)
            @as(T, null)
        else
            @as(T, try self.parseValue(arena, info.child, value)),
...
fn parseBoolean(self: Yaml, comptime T: type, value: Value) Error!T {
    _ = self;
    // LOCAL PATCH (zi): `Value.encode` of a bool default yields the `.boolean`
    // variant, but upstream parseBoolean only handled `.scalar` — so every
    // struct field `flag: bool = <default>` whose key is absent failed with
    // TypeMismatch. Accept `.boolean` directly.
...
fn parseOptional(self: Yaml, arena: Allocator, comptime T: type, value: ?Value) Error!T {
    // Both an absent key and a key with an empty value (`a:`) map to `null`.
    ...
    pub fn stringify(self: Value, writer: *std.Io.Writer, args: StringifyArgs) StringifyError!void {
        switch (self) {
            ...
            .list => |list| {
                ...
                if (first.isCompound()) {
                    for (list, 0..) |elem, i| {
                        // `writableSlice` asserts the buffer has room and
                        // cannot drain to make it; `splatByteAll` can.
        ...
        switch (tag) {
            ...
            .map_many => {
                ...
                for (0..map.data.map_len) |_| {
                    ...
                    if (out_map.contains(key)) return error.DuplicateMapKey;

                    // Build the value *before* inserting the key: inserting it
                    // first would hand a half-initialized entry (undefined
                    // value, key also owned by this scope's errdefer) to the
                    // outer errdefer if `fromNode` fails.
                    const value: Value = if (entry.data.maybe_node.unwrap()) |node|
                        try Value.fromNode(gpa, tree, node)
                    else
                        .empty;
